...
@router.get("/")
async def read_role(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user(allowed_roles=["admin"]))):
    data = await service.get_all_roles(db)
    return ApiResponse(message="Get all role data", data=data, current_user=current_user)

@router.post("/")
async def add_role(role: schemas.RoleCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user(allowed_roles=["admin"]))):
    data = await service.create_role(role, db)
    return ApiResponse(message=f"Create new role",data=data, current_user=current_user)

@router.put("/{role_id}")
async def update_existing_role(role_id: int, role: schemas.RoleUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user(allowed_roles=["admin"]))):
...
async def remove_role(role_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user(allowed_roles=["admin"]))):
